...
async function verificarPrazos() {
  ...
  const prazos = await prisma.prazo.findMany({
    where: {
      ...
      // Only check deadlines for active monitored processes (proper user scoping)
      processo: {
        monitoramentoAtivo: true,
        usuario: { ativo: true },
      },
  ...
  for (const prazo of prazos) {
    ...
    // Prevent duplicate notifications — check if we already notified about this prazo today
    const hojeInicio = new Date();
    hojeInicio.setHours(0, 0, 0, 0);
    const jaNotificou = await prisma.notificacao.findFirst({
      where: {
        usuarioId: prazo.processo.usuarioId,
        processoId: prazo.processoId,
        tipo: 'PRAZO',
        criadoEm: { gte: hojeInicio },
        mensagem: { contains: prazo.titulo },
      },
    });
    ...
    let mensagem;
    if (diasRestantes <= 1) {
      mensagem = `URGENTE: Prazo "${prazo.titulo}" vence AMANHA no processo ${prazo.processo.numero}`;
    } else {
    ...
    await notificationService.criarNotificacao({
      usuarioId: prazo.processo.usuarioId,
      processoId: prazo.processoId,
      titulo: diasRestantes <= 1 ? 'Prazo urgente!' : 'Prazo proximo',
      mensagem,
      tipo: 'PRAZO',
    });
  ...
  logger.info(`⏰ Process Monitor: ${alertasCriados} alertas de prazo criados (${prazos.length} prazos verificados)`);
}
