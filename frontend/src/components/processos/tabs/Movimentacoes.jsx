...
export default function TabMovimentacoes({ processo }) {
  ...
  useEffect(() => {
    ...
    // Immediately invalidate processos alerts so sidebar badge updates now
    qc.invalidateQueries({ queryKey: ['processos'] });
    qc.invalidateQueries({ queryKey: ['processos-alertas'] });
    // Mark notifications for this process as read
    api.patch(`/notificacoes/ler-por-processo/${processo.id}`).catch(() => {});
    qc.invalidateQueries({ queryKey: ['notif-count'] });
    // Also invalidate on unmount for good measure
    return () => {
      qc.invalidateQueries({ queryKey: ['processos'] });
      qc.invalidateQueries({ queryKey: ['processos-alertas'] });
    };
  ...
  return (
    <div>
      ...
      <div className="flex flex-col gap-3 mb-6">
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center gap-2">
            {/* Botao Sincronizar Removido conforme solicitacao */}
      ...
      {/* Form adicionar */}
      {showAdd && (
        <div className="rounded-xl p-4 mb-6 space-y-3 animate-fadeIn" style={{ background: 'rgba(255,255,255,.03)', border: '1px solid var(--border)' }}>
