...
function detectarTribunal(numeroCnj) {
  ...
  if (j === '8') { // Justiça Estadual
    const map = { 1:'TJAC',2:'TJAL',3:'TJAP',4:'TJAM',5:'TJBA',6:'TJCE',7:'TJDF',
  ...
  if (j === '6') { // Justiça Eleitoral
    const map = { 1:'TREAC',2:'TREAL',3:'TREAM',4:'TREAP',5:'TREBA',6:'TRECE',
...
export default function VincularCNJModal({ onClose, onSuccess, processoId }) {
  const [step, setStep] = useState(1); // 1: busca, 2: preview, 3: sucesso, 4: conflito
  const [form, setForm] = useState({ numeroCnj: '', tribunal: '' });
  const [tribunalDetectado, setTribunalDetectado] = useState(null);
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 overflow-y-auto" style={{ background: 'rgba(0,0,0,.85)', backdropFilter: 'blur(6px)' }} onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="w-full max-w-xl rounded-2xl overflow-hidden animate-scaleIn my-auto" style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border)' }}>
        ...
        <div className="p-6">
          ...
          {step === 4 && processoExistente && (
            <div className="space-y-4">
              ...
              <div className="rounded-xl p-4 space-y-2" style={{ background: 'var(--bg-tertiary)', border: '1px solid rgba(245,158,11,.2)' }}>
